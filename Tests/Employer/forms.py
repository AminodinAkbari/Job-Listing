...
class TestEmployerForms(TestCase):
    ...
    def test_edit_info_duplicate_phone(self):
    ...
    def test_edit_info_unvalid_and_duplicate_email(self):
        unvalid_email = EditEmailEmployer(data = {'email' : 'badmail'} , user = self.user)
        duplicate_email = EditEmailEmployer(data = {'email' : self.valid_manager['email']} , user=self.user)
        self.assertEqual(unvalid_email.errors['email'] , ['ایمیل معتبر نیست'])
